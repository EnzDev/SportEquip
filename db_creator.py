...
##Connextion with db
try:
    conn = mysql.connector.connect(host="localhost",user="root",password="root", database="SportEquip")
    cursor = conn.cursor()
...
def parse_activite():
    ...
    with open(get_path()[0])  as data_file:
        ...
        for iterator_activite in fichier_activite:
            ...
            print(ligne)
            cursor.execute( """INSERT IGNORE INTO activite (ComInsee, ComLib, EquipementId, ActCode, ActLib)  VALUES(%s, %s, %s, %s,%s)""",ligne)
            conn.commit()
...
def parse_installation():
    ...
    with open(get_path()[1])  as data_file:
        ...
        for iterator_installation in fichier_installation:
            ...
            address = iterator_installation["Numero de la voie"]+iterator_installation["Nom de la voie"] + iterator_installation["Nom de la commune"] + iterator_installation["Code postal"]
            print(address)
    ...
    with open(get_path()[2])  as data_file:
        ...
        for iterator_equipement in fichier_equipement:
            ...
            ligne = [iterator_equipement["ComInsee"], iterator_equipement["InsNumeroInstall"],
                     ...
                     iterator_equipement["EquGpsY"], iterator_equipement["EquGpsY"]]
            cursor.execute("""INSERT IGNORE INTO equipement (ComInsee, InsNumeroInstall, InsNomInstall, EquipementId, EquNom,EquGpsY,EquGpsX) VALUES(%s,%s, %s,%s,%s,%s,%s)""", ligne)
            conn.commit()
